...
def visualize_scattering_matrix():
    ...
    scattering_matrix = np.load(trim_config.SCATTERING_MATRIX_FILE)
    angle_weighted_matrix = np.einsum('ijk,i',scattering_matrix,trim_helper.ANGLE_WEIGHT)
    normalized_matrix = np.zeros(angle_weighted_matrix.shape)
    for damage_energy_idx in range(len(trim_helper.DAMAGE_ENERGIES)):
    ...
    plt.rcParams["figure.figsize"] = (0.9*4, 1*3)
    plt.rcParams["figure.dpi"] = 1000
    contour = plt.contourf(trim_helper.DAMAGE_ENERGIES, trim_helper.SIMULATED_SPECTRUM[:,0], normalized_matrix, np.linspace(0,1,100))
    plt.xscale('log')
    plt.yscale('linear')
    plt.xlim(1E-3, 10)
    plt.ylim(6E-1, 2)
...
if __name__ == "__main__":
    ...
    scattering_matrix = np.load(trim_config.SCATTERING_MATRIX_FILE)
    print(trim_helper.SIMULATED_SPECTRUM.shape)
